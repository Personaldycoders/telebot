...
async function uploadImageToTelegraph(filePath) {
    ...
    try {
        ...
        writer.on('finish', async () => {
            try {
                ...
                dy.sendMessage(chatId, `Gambar berhasil diunggah: ${telegraphUrl}`);
                fs.unlinkSync(filePath);
            } catch (error) {
                dy.sendMessage(chatId, 'Gagal mengunggah gambar ke telegra.ph');
            }
        });

        writer.on('error', () => {
            dy.sendMessage(chatId, 'Gagal mengunduh gambar dari Telegram');
        });
    } catch (error) {
        dy.sendMessage(chatId, 'Terjadi kesalahan. Coba lagi nanti.');
    }
});
...
function promptForToken() {
  ...
  rl.question('Please enter your bot token: ', (token) => {
    ...
    fs.writeFile(configPath, JSON.stringify(config), (err) => {
      ...
      console.log('Token successfully saved to config.json');
      startBot(token);
      rl.close();
    });
  });
  ...
  if (err) {
    ...
    fs.readFile(configPath, 'utf8', (err, data) => {
      ...
      try {
        ...
        if (!token) {
          console.error('Bot token is required in config file!');
          process.exit(1);
        }
        startBot(token);
      } catch (err) {
